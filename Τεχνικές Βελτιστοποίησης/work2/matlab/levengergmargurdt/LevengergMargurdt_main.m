...
startingPoint  = [0 ; 0];
[x,k,iter] = LevengergMargurdt_constant(0.0001,startingPoint,f,grad,hessi);
figure;
plot(x(1,:),x(2,:))
...
title(['$constant \enspace \gamma = 0.25 \enspace with \enspace iterations = $' a '\enspace at \enspace point (' x_1 ',' x_2 ') \enspace with \enspace value \enspace f(x,y)=' f_min],'Interpreter', 'latex','FontSize',25)
hold on
contour(x_, y_, z_)
colorbar
...
startingPoint  = [1 ; 1];
[x,k,iter] = LevengergMargurdt_constant(0.0001,startingPoint,0.25,grad,hessi);
figure;
p = plot(x(1,:),x(2,:));
...
% startingPoint  = [0 ; 0];
% [x,k,iter] = LevengergMargurdt_min(0.0001,startingPoint,f,grad,hessi);
% figure;
% plot(x(1,:),x(2,:))
%  
% a = num2str(iter); 
...
startingPoint  = [0 ; 0];
[x,k] = LevengergMargurdt_armijo(0.0001,startingPoint,f,grad,hessi);
figure;
plot(x(1,:),x(2,:))
if k>1
a = num2str(k-1);
...
startingPoint  = [1 ; 1];
[x,k] = LevengergMargurdt_armijo(0.0001,startingPoint,f,grad);
figure;
plot(x(1,:),x(2,:))
a = num2str(k-1);
x_1 = num2str(x(1,k));
